...
public class BoardController extends GenericController<Board> {
    ...
    private void doList(Request request, Response response) {
        ...
        try (Connection con = DriverManager.getConnection("jdbc:mysql://localhost:3306/studydb", "study", "1111");
            ...
            while(rs.next()) {
                out.printf("%d, %s, %s, %s, %d\n", rs.getInt("no"), rs.getString("title"), rs.getString("conts"), rs.getDate("regdt"), rs.getInt("vwcnt"));
            }
    ...
    private void doAdd(Request request, Response response) {
        ...
        try (Connection con = DriverManager.getConnection("jdbc:mysql://localhost:3306/studydb", "study", "1111");
                PreparedStatement pstmt = con.prepareStatement("insert into ex_board(title,conts,regdt,vwcnt) values(?,?,now(),0)");
                ) {
