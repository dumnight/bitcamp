...
public class BoardDaoImpl implements BoardDao {
            ...
            ArrayList<Board> list = new ArrayList<>();
                ...
                while(rs.next()) {
                    Board board = new Board(
                            rs.getInt("no"), rs.getString("title"), rs.getString("conts"), rs.getInt("vwcnt"), rs.getDate("regdt"));
                    list.add(board);
                }
                return list;
    ...
    public int insert(Board board) {
        ...
        try {
            con = ds.getConnection();
            pstmt = con.prepareStatement("insert into ex_board(title,conts,regdt,vwcnt) values(?,?,now(),0)");
