...
public class DispatcherServlet extends HttpServlet {
        ...
        for (int i = 0; i < params.length; i++) {
            if (params[i].getType() == HttpServletRequest.class) {
                ...
                if (anno != null) {
                    ...
                    values[i] = createParamTypeObject(params[i], request);
                }
            }
    ...
    private Object createParamTypeObject(Parameter parameter, HttpServletRequest request) {
    ...
    private Object toParamTypeValue(Parameter param, String value) {
        ...
        //파라미터의 타입에 따라 String 값을 그 타입의 값으로 형변환 한다
        if (type == byte.class) {
