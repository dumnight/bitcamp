...
public class Test12 {
    ...
    //기본 타입외에 다른 타입으로 형변환을 요구하면 프론트 컨트롤러는 오류를 발생시킨다.
    //해결방안 => String을 java.util.Date 객체로 변환시켜주는 변환기를 만들어 스프링에 꼽아야 한다.
    //변환기를 꼽는 방법
    // 1)해당 페이지 컨트롤러에 @InitBinder 메서드를 둔다.
    // 2)@ControllerAdvice 클래스에 @InitBinder 메서드를 둔다.
    // 3)WebBindingInitializer 구현체를 제공한다.
    @RequestMapping("m2")
    ...
    // @InitBinder
    // 프론트 컨트롤러(DispatcherServlet)는 클라이언트로 부터 요청이 들어오면 클라이언트가 보낸 데이터를 적절하게 변환하기 위해
    ...
    // @InitBinder가 붙은 메서드는 프론트 컨트롤러가 문자열을 특정 타입의 값으로 바꾸기 전에 WebDataBinder를 추가로
    // 설정하기 위해 호출하는 메서드 이다.
    // 문자열을 다른 타입의 값으로 바꾸기 전에 매번 호출 된다
    // 즉 요청 핸들러의 파라미터가 5개면 요청할때 5번 호출된다.
    
    // 방법1 : @InitBinder 메서드를 페이지 컨트롤러에 두기
    // 이 메서드는 오직 이 페이지 컨트롤러에만 영향을 미친다.
    /*
    @InitBinder
    protected void initBinder(WebDataBinder binder) {
        System.out.println("Test12_InitBinder()");
        //문자열을 날짜 객체로 만들어줄 도구를 준비
        SimpleDateFormat dateFormat = new SimpleDateFormat("yyyy-MM-dd");
        dateFormat.setLenient(false); //날짜 형식을 엄격하게 검사
        //스프링에서 제공하는 문자열을 날짜 객체로 변환시켜주는 커스텀 에디터를 준비하여 WebDataBinder에 등록한다.
        binder.registerCustomEditor(Date.class,           // 문자열을 어떤 타입으로 바꿀지 결정한다. 
                                    new CustomDateEditor( // 문자열을 java.util.Date객체로 만들어 준다
