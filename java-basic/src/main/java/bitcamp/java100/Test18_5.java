...
public class Test18_5 {
    ...
    public static void main(String[] args) throws Exception {
        ...
        String s2 = s1.replace("BC", "HAHA");
                System.out.println(s1);
                System.out.println(s2);
                
                //변경할 수 없는 인스턴스를 immutable object라고 부름
                ...
                //indexOf(찾을 문자열)
                //앞에서 부터 문자열찾아 찾은 위치의 인덱스를 리턴한다
                System.out.println(s1.indexOf("BB"));               
                               
                //lastIndexOf(찾을 문자열)
                    
                s1.lastIndexOf(s1.lastIndexOf("BB"));
                
                //substring(추출할 문자열의 시작 인덱스
                //지정된 인덱스의 문자부터 끝까지 문자열을 추출
                System.out.println(s1.substring(3));
                
                //substring
                //시작 인덱스 부터 끝 바로 직전 문자까지 추출
                // 끝 인덱스의 문자는 포함하지 안는다
               ...
               //compairTo(비교문자열)
               //두 문자열을 비교하여 같으면 0을 리턴
               //알파벳/가나다 순서에서 문자열1이 문자열 2보다 앞서면 음수를 리턴한다
               ...
               //UpperCase,LowerCase
               //원본은 바뀌지않는다
               //s1을 소문자로 바꾼 새로운 String 인스턴스가 Heap에 만들어지고 그 주소가 출력된다
               s1 = "Hong Gil Dong";
               System.out.println(s1.toLowerCase());
               System.out.println(s1.toUpperCase()); //대문자로 변경
               
               //trim
               //문자열의 양쪽 공백을(white space) 제거한다(문자열 사이의 공백은 허용)
               s1 = "  \t홍\t길 동 \n입니다. \n\n\t\n\n\n\n";
               System.out.print(">");
               ...
               //공백(white space)
               //space bar를 눌러 추가한 빈 칸 한개, 탭, 줄바꿈 모두 공백으로 취급
               
               s1 = "ABC\n가각간\n똘똠똥";
               
               //getBytes() 
               //해당 문자열을 Byte 배열로 만들어 리턴한다
               //Byte 배열에 저장되는 값은 JVM에 설정된 문자집합의 코드 값이다
               ...
               // 문자열을 다루는 특별한 연산자 '+'
               // 두 문자열을 연결하여 새 String 인스턴스를 생성
               
               s1 = "ABC";
               s2 = "가각간";
               s3 = s1 + s2;
               //s1 + s2는 주소를 더하는것이 아니라 두 인스턴스의 값을 연결하여 새 String 객체를 만드는것
               System.out.println(s1);
               System.out.println(s2);
               System.out.println(s3);
