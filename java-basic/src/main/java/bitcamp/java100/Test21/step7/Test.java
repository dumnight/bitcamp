...
package bitcamp.java100.Test21.step7;

import java.io.Console;
// 7단계 배열에 객체의 주소를 저장하는 부분을 메서드로 분리한다. 
public class Test {
        ...
        static void printContacts() {                 // 분리 1번째 순서
            //배열에 저장된 값을 출력한다.
            for(int i = 0; i < size(); i++) {         //8번째 cursor -> size
        ...
        static void add(Contact contact) {              //분리 5번째 순서
            contacts[cursor] = contact;
            cursor++; //증가시킬 값은 입력후에 위치하게 한다 
        }
        
        static Contact get(int index) {                    //6번째 순서
            return contacts[index];
        }
        
        static int size() {                                //7번째 순서
            return cursor;
        }
