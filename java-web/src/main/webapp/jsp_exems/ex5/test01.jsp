...
<h3>language 속성</h3>
<p>
JSP 스크립트에서 사용할 프로그래밍 언어를 지정한다.<br>
원래는 java 뿐만 아니라 다른 프로그래밍 언어도 수용하려 하였다.<br>
JSp를 java에 종속된 기술이 아니라 언어에 독립된 기술려 하였다.<br>
그러나 지금까지 Java만 지원하고 있다<br>
한 때 개발자들이 java이외의 언어도 지원하도록 오픈 프로젝트를 진행한 적이 있다.<br>
그러나 지금은 모두 폐기 되었다<br>
</p>

<h3>contentType 속성</h3>
<p>
출력하는 콘텐츠의 MIME 타입과 문자집합을 설정하는 코드를 생성한다<br>
즉 다음의 자바 코드를 생성한다.<br>
</p>
...
JSP 파일을 저장할 때 'UTF-8'로 저장했으면, 이 속성의 값도 'UTF-8'로 지정하라<br>
생략해도 된다.<br>
</p>
 
<h3>trimDirectiveWhitespaces 속성</h3>
<p>
지시부 태그들 사이에 삽입된 공백(스페이스, 줄바꿈, 탭 등)을 출력문으로 바꾸지 않는다<br>
</p>

<h3>autoFlush 속성</h3>
<p>
출력한 내용이 콘텐츠가 버퍼에 꽉찼을 때 자동으로 출력할지를 결정한다<br>
true 이면, 자동으로 출력한다. true가 기본 값이다.<br>
false로 설정하면 , 버퍼가 찼을 때 실행 오류를 발생시킨다.<br>
즉 false로 설정했을 때는 개발자가 버퍼가 차기 전에 명시적으로 flush()를 호출하여 
버퍼의 내용을 출력해야 한다<br>
</p> 
...
한번에 보내고 싶다면 버퍼의 크기를 증가시켜라 <br>
</p>

<h3>errorPage 속성</h3>
<p>
JSP페이지를 실행하다가 오류가 발생하면 서블릿 컨테이너에 설정된 페이지가 출력된다 <br>
이를 바꾸고 싶다면 이 속성에 다른 페이지의 URL을 입력하라 <br>
그러면 에러가 발생했을때 해당 URL의 서블릿이나 JSP로 포워딩 할 것이다.
</p>

<h3>import 속성</h3>
<p>
서블릿 클래스에 import문을 추가할 때 사용한다<br>
import 속성은 값을 설정하면 다음과 같이 자바 코드를 생성한다.<br>
</p>
